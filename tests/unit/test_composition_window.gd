...
func after_each() -> void:
	...
	SettingsManager.set_setting("composition/profiles", _saved_profiles if _saved_profiles != null else [])
...
func test_summary_label_updates() -> void:
	var profiles: Array = [
		DroneProfile.create("A", FleetData.DroneType.DRONE_RIFF, &"", [], 300).to_dict(),
		DroneProfile.create("B", FleetData.DroneType.DRONE_EMO, &"", [], 200).to_dict(),
	]
	SettingsManager.set_setting("composition/total_drones", 1000.0)
	SettingsManager.set_setting("composition/profiles", profiles)
	_window.open()
...
func test_delete_profile_removes_from_draft() -> void:
	var profiles: Array = [
		DroneProfile.create("OnlyOne", 0, &"", [], 100).to_dict(),
	]
	SettingsManager.set_setting("composition/profiles", profiles)
	_window.open()

	# Il doit y avoir au moins un enfant dans ProfilesContainer
	var container = _window.get_node("%ProfilesContainer")
	assert_gt(container.get_child_count(), 0)

	# Simuler la suppression interne
	_window._on_delete_profile(0)
	assert_eq(_window._draft_profiles.size(), 0)
